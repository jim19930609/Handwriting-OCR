...
def conv_layer(inpt, filter_shape, stride):
    ...
    moving_mean = tf.assign(moving_mean, tf.add(alpha*moving_mean, (1-alpha)*mean))
    moving_var = tf.assign(moving_var, tf.add(alpha*moving_var, (1-alpha)*var))
...
def resnet(inpt, n, cifar):
    ...
    with tf.variable_scope('fc'):
        ...
        flat_size = int(shape[1] * shape[2] * shape[3])

        innerproduct = fc_layer(flat_conv, [flat_size, cifar])
